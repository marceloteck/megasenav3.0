...
import os   
os.system('title Lotofacil Trainer v1.4')
...
if newTrainer == "noTrainner":
    model_path = f'LOTOFACIL/Trainner/lotofacil_model_training.keras'
else:
   model_path = f'LOTOFACIL/Trainner/lotofacil_model_training.{current_time}.keras'   
...
history = best_model.fit(X_train_scaled, y_train_scaled, epochs=500, validation_split=0.2, batch_size=16, callbacks=[EarlyStopping(monitor='val_loss', patience=10)])
...
most_frequent_numbers = number_counts.head(90).index.tolist()  # Selecionar os 30 números mais frequentes
...
def plot_predictions(y_true, y_pred):
    ...
    for i in range(6):  # LOTOFACIL tem 6 números por sorteio
        plt.subplot(3, 2, i + 1)
