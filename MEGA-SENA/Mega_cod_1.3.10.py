...
def treinar_modelo_basico(modelo, entradas, saidas, dados):
    ...
    for i in range(len(entradas)):
        ...
        # Exibição do progresso a cada iteração
        if i % 10 == 0:  # A cada 10 iterações, exibe informações
        ...
        # Fit retorna o histórico do treinamento, que pode ser usado para monitorar o progresso
        historico = modelo.fit(entradas, saidas, epochs=1, verbose=0)

        # Salvar a perda da época atual para posterior análise
    ...
    # Visualizar os pesos da rede neural após o treinamento (opcional)
    for i, camada in enumerate(modelo.layers):
        pesos = camada.get_weights()
        print(f"[INFO] Pesos da camada {i}:", pesos[0] if len(pesos) > 0 else "Sem pesos")
