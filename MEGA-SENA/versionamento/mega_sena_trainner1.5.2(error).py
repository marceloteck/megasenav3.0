...
def prepare_data(data, X_filename, y_filename, force_prepare=False):
    ...
    if not data.empty:
        ...
        y_new = np.array([data.iloc[i, 1:7].values for i in range(NTime, len(data))])  # Ajuste para capturar 6 números
...
# Função para adicionar features (sem remover a coluna 'Data' permanentemente)
def add_features(df):
    df_temp = df.drop(columns=['Data'], errors='ignore')  # Remover 'Data' temporariamente
    ...
    for i in range(1, 61):
        df_temp[f'freq_{i}'] = frequency[i]

    df_temp['mean'] = df_temp.mean(axis=1)
    df_temp['std'] = df_temp.std(axis=1)

    return pd.concat([df[['Data']], df_temp], axis=1)  # Reinserir 'Data'
