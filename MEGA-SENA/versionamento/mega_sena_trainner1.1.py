...
def load_and_preprocess_data(folder_path, processed_files_filename='MEGA-SENA/processed_files.txt'):
...
"""
def prepare_data(data, X_filename='MEGA-SENA/prepared_data_X.npy', y_filename='MEGA-SENA/prepared_data_y.npy', force_prepare=False):
...
def prepare_data(data, X_filename='MEGA-SENA/prepared_data_X.npy', y_filename='MEGA-SENA/prepared_data_y.npy', processed_files_filename='MEGA-SENA/processed_files.txt', force_prepare=False):
    if not force_prepare and os.path.exists(X_filename) and os.path.exists(y_filename):
        print("Carregando dados preparados dos arquivos...")
    ...
    if os.path.exists(processed_files_filename) and not data.empty:
        print("Adicionando novos dados...")
        X_new, y_new = [], []
        for i in tqdm(range(5, len(data)), desc="Preparando dados", unit="iteração"):
            X_new.append(data.iloc[i - 5:i, 1:].values.flatten())
...
history = best_model.fit(X_train_scaled, y_train_scaled, epochs=1, validation_split=0.2, batch_size=16, callbacks=[EarlyStopping(monitor='val_loss', patience=10)])
...
# Normalizando e ajustando a forma para (1, 30, 1)
